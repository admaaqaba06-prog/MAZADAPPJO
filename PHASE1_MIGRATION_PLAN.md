...
# ملحق: تحليل الترحيل — وثلاثة تصحيحات لاقتراحاتي أنا

> اكتمل بعد كتابة ما فوق. **يصحّح ثلاثة أشياء اقترحتُها، ويكشف خطرين صنعهما كودٌ نشرتُه اليوم.**

## ✅ الفرضية المركزية صحيحة

**لا حاجة إلى backfill إطلاقاً** — اختُبرت على الحالات العشر، وصمدت في كلها. كل سجل قائم إمّا نهائي، أو ما زال يطابق نفس الاستعلام الذي يطابقه اليوم، أو يحمل كل الحقول التي يقرأها كود المسار ب المحفوظ.

**لكن الذي يُكسَر ليس البيانات — بل قابلية الوصول.**

## 🔴 تصحيح ١ — `isOfficialWinner` كما اقترحتُها خاطئة

المبيعات السابقة للنشر **لا تحمل `belowReserveOffer` ولا `sellerAcceptedAt` إطلاقاً**. فأي مُحدِّد يشترط وجود كائن عرض مقبول **سيصنّف كل بيعة تاريخية بأنها ليست فوزاً**.

> **الصياغة الصحيحة:** `status === 'completed'` **أو** وجود طلب — **ولا تشترط وجود العرض أبداً**.

## 🔴 تصحيح ٢ — كتابة `ended` عند الرفض ستنشر بيوعاً وهمية بأثر رجعي

اقترحتُ أن يذهب المزاد المرفوض/المنقضي إلى `ended`. توقّعت أن يضرّ المزادات الجديدة. **الحقيقة أسوأ:** كتابة `auction.status='ended'` غير مشروطة **ستعيد كتابة مستندات `reserve_not_met` القديمة** فتدخل استعلام «الفوزات الأخيرة» في `useSocialProof` — **فتُعلَن بيوع لم تحدث قط، من التاريخ.**

> **العلاج:** لا تلمس `auction.status` على الرفض/الانقضاء إطلاقاً — أو أصلح `useSocialProof` أولاً ليستعلم من **الطلبات**.

## 🔴 تصحيح ٣ — `notify()` لا يفرض `idempotencyKey`

فحصتُه بنفسي: **`functions/notify.js` لا يذكر `idempotencyKey` ولا مرة واحدة.** إنه حقل من جانب n8n فقط ⟹ **جرس التطبيق وإشعارات FCM بلا أي منع تكرار.**

⟹ أي مسار قبول يتخلّى عن قصر `orderSnap.exists` **سيعيد إطلاق `auction_won` داخل التطبيق.**

## 🔴🔴 والأخطر: التراجع محفوف — بسبب كودي أنا

مزاد عالق على الحالة الجديدة وقت التراجع:

1. **غير مرئي للكود القديم** — لا يطابق أي مرشّح شاشة.
2. **`authorizeOrderRepair`** — كتبتُها اليوم في #292 — **ترفضه بـ`offer_pending`**، فالعلاج اليدوي الوحيد للبناء القديم **مسدود بكودي**.
3. **`expireLapsedOffers`** — نشرتُها اليوم — ستقول لأولئك المزايدين بعد ٢٤ ساعة **«لم يقبل البائع مزايدتك»** على قطع **بلغت حدّها الأدنى فعلاً**، بلا أي تدخل بشري.

> **⟹ سكربت الإصلاح ليس خياراً احتياطياً. يُكتب ويُختبر قبل النشر، لا بعده.**

## 🟠 الحالة الجديدة لا تطابق أي شاشة

| السطح | ما يحدث |
|---|---|
| `useMyAuctionLots.ts:60` | يصل كـ`removed` ⟹ لا كونفيتي **(مقصود ✓)** |
| `OurDropsSection.tsx:281` | **يختفي عن الأدمن كلياً** |
| `DropsListPanel.tsx:14,95` | لا يطابق أي تبويب، **ويطبع رمز الحالة الخام للأدمن** |
| `sellerListings.ts:57-71` | لا يقع في أي سلة بائع — يظهر تحت «الكل» فقط |
| `SellerCenterView.tsx:115` | **لا ترجمة ⟹ يُوسَم «قيد المعالجة»** — قطعة مغلقة موسومة «قيد المعالجة» |
| `auctionTimeStatus.ts:72` | يتدهور بأمان ✓ |

## 🟠 حارس الترسية المزدوجة لا يعرف الحالة الجديدة

`index.js:446` يفحص `['completed','ended','reserve_not_met']` فقط. الحماية اليوم **عرضية** لا مقصودة. وأسوأ: حارس `settledAt` الذي أضفتُه في #292 سيطبع **«أُعيد فتحه بعد التسوية»** لقطعة **لم تُبَع قط**. يجب إضافة الحالة الجديدة صراحةً.

## 🟠 بوابات القبول/الرفض تبقى على حالة **العرض**

اليوم `offerAwaitsSeller` (`reserveStatus.ts:78`) والخادم (`index.js:3480`) كلاهما **لا يقرأ حالة المزاد**. لو اشترط النداء الجديد `auction.status === 'pending_seller'`، **يصبح كل عرض جارٍ غير قابل للقبول لحظة النشر** — يختفي زر البائع، وبعد ٢٤ ساعة يُنهيه الـsweep.

> **قاعدة مُلزِمة: البوابات تبقى على `belowReserveOffer.status`، لا على `auction.status`.**

## 🟡 الحجم

القوائم مقيَّدة (`ADMIN_AUCTIONS_CAP`، `limit(100)` للطلبات). اليوم العروض نادرة — بعد المرحلة ١ **كل مزاد مغلق يحمل قراراً معلّقاً**. النوافذ المقيَّدة تصير مشكلة حقيقية.

## ملاحظة على النزاهة

**الناقد المضادّ لم يُنفَّذ** — فشل بخطأ في سكربتي (استعملت `require` وبيئة الـworkflow بلا Node APIs). ما فوق مصدره وكيل الترحيل زائد تحققي المباشر. أستطيع تشغيله إن أردت طبقة تدقيق إضافية.
