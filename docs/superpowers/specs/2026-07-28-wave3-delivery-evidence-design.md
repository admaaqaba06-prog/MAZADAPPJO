...
3. **Code visibility:** the delivery code is seller+admin-visible and **hidden from the buyer** via Firestore rules field-level denylist (buyer learns it only from the physical parcel/handover). Mirror Wave 1's payment-proof lockdown.
