...
**A blocked runner-up does not qualify** (review F1). The accept callable refuses `buyer_accept` from a restricted account, and the payment-default ban minimum is 48h against a 24h offer window — so an offer opened to a blocked bidder is *deterministically* doomed: an Accept button the server will always refuse, while the lot is held out of auto-relist for 24h on behalf of someone who can never take it. With 21 of 31 orders defaulted, a meaningful share of runner-ups are under an active block, very often for defaulting elsewhere. The enforcer therefore reads the runner-up's user doc and skips the lot when the account is blocked — still **no cascade** to the third bidder, so a skipped lot is simply the no-qualifying-bidder case above. The **read** is at the `index.js` call site (I/O, which the pure layer does not do; and one read for the chosen bidder beats a read per bidder to build a ban set before the pick), while the **judgement** is `secondChance.shouldSkipRunnerUp`, so it is covered behaviourally rather than by source greps — an earlier inline version was mutated to its exact inverse and every source-text assertion still passed. The client mirrors the rule in `secondChanceViewState` (accept hidden, decline kept, and the card says why) for the ban that lands *after* the offer was opened.

**The surprising part: the ban read FAILS OPEN.** If the `users/{uid}` lookup throws, the offer is opened anyway. That is deliberately the *opposite* call to the reserve lookup a few lines below it, and the two are not inconsistent — both fail in the direction that **preserves the option**, they simply have different costs:

| Read | On error | Why |
| --- | --- | --- |
| reserve (`auctionSecrets`) | ask the seller | failing the other way sells a lot under its seller's reserve with no consent — the exact harm the fork exists to prevent |
| ban (`users/{uid}`) | open the offer | failing the other way permanently destroys a legitimate runner-up's one and only shot: a `defaulted` order never re-enters the enforcer's `waiting_payment` query, and a second chance is one-shot |

Failing open here costs at most a re-run of the behaviour that shipped before the check existed, because **the accept-time gate is a complete backstop** — `secondChanceRespond` reads the bidder doc inside its transaction, so that check is atomic rather than TOCTOU. A missing user doc is treated the same way, and for the same reason: no user record is not evidence of a ban.
