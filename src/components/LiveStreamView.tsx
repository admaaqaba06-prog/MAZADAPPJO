...
export const LiveStreamView: React.FC = () => {
  ...
  const handleSaveToggle = () => {
    setIsSaved(!isSaved);
    triggerToast(isSaved ? 'Removed from watchlist' : 'Saved to watchlist! 🔖');
  };
        ...
        <div className="flex flex-col items-center gap-0.5 font-sans">
          <button 
            ...
            style={{
              ...
              backdropFilter: 'blur(8px)',
              WebkitBackdropFilter: 'blur(8px)',
              border: isSaved ? '1px solid rgba(255,107,0,0.4)' : '1px solid rgba(255,255,255,0.15)',
              borderRadius: '50%',
              color: 'white'
            }}
          >
      ...
      >
        ...
        <div className="pointer-events-auto space-y-1">
          ...
          <div className="space-y-1.5 py-1.5 pointer-events-auto">
                ...
                return (
                  <button
                    ...
                    onClick={() => {
                      ...
                      triggerToast(isAr ? `تعديل زيادة المزايدة الفورية بقيمة +${val} د.أ` : `Bid increment modified: +${val} JOD`);
                      setTimeout(() => {
            ...
            {/* Solid Orange Primary Bidding CTA */}
            <button
              type="button"
              onClick={() => {
                executeBid(bidCTAAmount);
                setIsMainBidClicked(true);
                setTimeout(() => {
                  setIsMainBidClicked(false);
                }, 1000);
              }}
              disabled={currentUser.isBlocked || wallet.availableBalance < selectedIncrement}
              className={`w-full bg-[#FF6B00] hover:bg-orange-600 text-white font-bold h-[48px] rounded-[14px] text-sm flex items-center justify-center gap-1.5 transition-all active:scale-[0.98] disabled:opacity-45 select-none shrink-0 cursor-pointer ${
                isMainBidClicked ? 'main-bid-clicked-anim main-bid-glow' : ''
              }`}
              id="main-lot-bid-submit-cta"
            >
              {isMainBidClicked ? (
                <Check className="w-4.5 h-4.5 text-white stroke-[3] animate-bounce" />
              ) : (
                <Gavel className="w-4.5 h-4.5 text-white" />
              )}
              <span>
                {isAr 
                  ? `زايد ${bidCTAAmount.toLocaleString()} JOD` 
                  : `BID ${bidCTAAmount.toLocaleString()} JOD`}
              </span>
            </button>
          </div>
