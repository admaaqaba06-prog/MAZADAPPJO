    ...
    >
      ...
      >
        ...
        <label className="block text-[11px] font-semibold text-gray-500 mb-1">
          {tokenLabel}{' '}
          <span className="font-mono font-bold text-gray-800">{confirmToken}</span>
        </label>
        <input
          value={typed}
          onChange={(e) => setTyped(e.target.value)}
          autoFocus
          className="w-full border border-gray-200 rounded-xl px-3 py-2 text-sm font-mono mb-3 focus:outline-none focus:ring-2 focus:ring-red-200"
        ...
        {requireReason && (
          <>
            ...
            <textarea
              ...
              rows={2}
              className="w-full border border-gray-200 rounded-xl px-3 py-2 text-xs mb-3 resize-none focus:outline-none focus:ring-2 focus:ring-red-200"
        ...
        <div className="flex gap-2">
          <button
            onClick={onCancel}
            disabled={busy}
            className="flex-1 py-2.5 rounded-xl text-xs font-bold text-gray-600 bg-gray-100 hover:bg-gray-200 transition-colors disabled:opacity-50"
          >
            {isAr ? 'إلغاء' : 'Cancel'}
          </button>
          <button
