...
// dead. This hook fixes that by scoping to MY lots and DELIBERATELY including
// the terminal statuses in the `status in [...]` list, so a lot I'm winning
// STAYS in the result when it completes (delivered as `modified`, not
// `removed`). The `live→completed` transition is therefore observable — the
// exact thing the broad `[live,upcoming]` query denies — which is what lets the
// per-id transition rule in `useWinDetection` fire the celebration. This
// activation is intended and approved.
//
...
export function useMyAuctionLots(userId: string | null | undefined): AuctionItem[] {
  ...
  useEffect(() => {
    ...
    const q = query(
      ...
      where('status', 'in', ['live', 'completed', 'ended', 'reserve_not_met']),
      limit(20),
    );
